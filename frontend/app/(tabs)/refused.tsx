...
const Refused = () => {
  ...
  const { filteredRefused } = useData();
  const [refusedData, setRefusedData] = useState<any[]>([]);
  const { t } = useTranslation();

  useEffect(() => {
    setRefusedData(filteredRefused);
  }, [filteredRefused]);
        ...
        <View className="flex-1 pb-[100px]">
          {refusedData?.length > 0 ? (
            ...
            <View className="w-full h-full min-h-[80vh] items-center flex-1 justify-center">
              <Text className="text-xl text-center">{t("noRefused")}</Text>
